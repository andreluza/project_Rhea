...
load(here("data","organized_data", "input_GBIF.RData"))

## inat
load(here("data","organized_data", "input_INAT.RData"))

## vertnet
load(here("data","organized_data", "input_VERTNET.RData"))

## ebird
load(here("data","organized_data", "INPUT_ebird.RData"))
...
## transforming and standardizing covariates
...
plot(area_mun,numero_ovelhas$Ovinos,main="Sheep")
plot(area_mun,numero_bovinos_corte$n_cabecas, main = "Cattle")
plot(area_mun,numero_estabelecimentos_lavoura$Soja_em_grao, main = "Soybean")
...
plot(area_mun,agricultura_uso, main="Agriculture")
...
# plot it
plot(area_mun,habitat_lavoura,xlab="Municipality area",ylab= "Area (km2)",main = "Crop field")
plot(area_mun,habitat_floresta,xlab="Municipality area",ylab= "Area (km2)",main="Forest")
plot(area_mun,habitat_campo,xlab="Municipality area",ylab= "Area (km2)",main="Grassland")
...
plot(area_mun,densidade_gado,xlab="Municipality area",ylab= "Density (sheep)")
plot(area_mun,densidade_ovelha,xlab="Municipality area",ylab= "Density (cattle)")
...
plot(area_mun,
     ...
     xlab="Municipality area",ylab= "Cover")
...
plot(area_mun,
 	...
 	main="Forest-like habitats", xlab="Municipality area",ylab= "Area")
...
c_sem <-   ggplot() + geom_polygon(data=f.mun_sem, aes(x=long, y=lat, group=group, 
  ...
  scale_fill_gradient2 (low='white', high='darkred', na.value = "white",
                        ...
                        breaks=seq(0,max(cores_sem$cores,na.rm=T),by=2),
...
load(here("data","organized_data", "CARparams.RData"))
...
# select a number of sampling occasions (SO) for ebird (too much sampling events (623) in just a couple of sites)
n_so_ebird <- 11 ## 11 is the number of SO for which most municipalities have data
# bound data (with space) and report results (list across spatial data (different cell sizes))
out_space_grassland_model  <- lapply (seq (1,length(adj)), function (i) {
	...
	tryCatch ( # try overcome errors
	  out_space_no_cattle <- bugs(data = win.data.space, parameters.to.save = params.espaco, 
...
# write model
sink("static_model_data_integration_sem_espaco.txt")
cat("
...
# --------------
# list with all model's results

res_occ_models<-c(list(out_sem_espaco_sem_gado),
			out_space_grassland_model)

# save it
save (res_occ_models,file=here("output", "res_occ_models.RData"))
